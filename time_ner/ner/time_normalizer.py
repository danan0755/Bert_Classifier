...
from ner.string_prehandler import StringPreHandler
from ner.time_point import TimePoint
from ner.time_unit import TimeUnit
...
class TimeNormalizer:
    ...
    def _filter(self, input_query):
        ...
        input_query = input_query.replace('号', '号，')
        input_query = input_query.replace('日', '日，')
        input_query = input_query.replace('大概', '')

        input_query = input_query.replace("－", "-")
        rule = u"[0-9]{1,}\-[0-9]{1,}点"
        pattern = re.compile(rule)
        fd = pattern.findall(input_query)  #输入['9-10点']
        if fd:
            sp = str(fd[0]).split('-')
            fd_change = sp[0] + "点到" + sp[1]
            input_query = input_query.replace(fd[0], fd_change) # 返回['9点-10点']

        rule = u"[0-9]{1,}到[0-9]{1,}点"
        pattern = re.compile(rule)
        fd = pattern.findall(input_query)  # 输入['9到10点']
        if fd:
            sp = str(fd[0]).split('到')
            fd_change = sp[0] + "点到" + sp[1]
            input_query = input_query.replace(fd[0], fd_change)  # 返回['9点-10点']
    ...
    def parse(self, target, timeBase=arrow.now()): # 这里最好不要给timeBase赋值默认时间，不然调用时会忘记赋值
        """
        ...
        if self.isTimeSpan:
            ...
            if len(res) == 0:
                ...
                dic['type'] = 'timestamp'
                # 补充：输入句子没有小时则不输出HH:mm:ss
                if res[0].tp_origin.tunit[3] < 0:
                    dic['timestamp'] = res[0].time.format("YYYY-MM-DD")
                    dic['timetype'] = res[0].time_type
                else:
                ...
                # 补充：输入句子没有小时则不输出HH:mm:ss
                if res[0].tp_origin.tunit[3] < 0:
                    time1 = res[0].time.format("YYYY-MM-DD")
                    dic['timetype'] = res[0].time_type
                else:
                    time1 = res[0].time.format("YYYY-MM-DD HH:mm:ss")

                if res[1].tp_origin.tunit[3] < 0:
                    time2 = res[0].time.format("YYYY-MM-DD")
                    dic['timetype'] = res[1].time_type
                else:
                    time2 = res[1].time.format("YYYY-MM-DD HH:mm:ss")

                if time1 in time2:
                    dic['timespan'] = [time2]
                else:
                    dic['timespan'] = [time1, time2]
