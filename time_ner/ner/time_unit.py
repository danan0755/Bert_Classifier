...
from ner.range_time_enum import RangeTimeEnum
from ner.lunar_solar_converter import *


# 时间语句分析
class TimeUnit:
    def __init__(self, exp_time, normalizer, contextTp):
        # 新增类型，凌晨，早上，中午，下午，晚上，对应-2，-3，-4
        self.time_type = 0
        self._noyear = False
    ...
    def genTime(self, tunit):
        ...
        if tunit[3] > 0:
            if tunit[3] == 24:
                tunit[3] = 0
                time = time.replace(day=tunit[2] + 1)
    ...
    def norm_sethour(self):
        ...
        rule = u"凌晨|早上|早晨|早间|晨间|今早|明早|早|清晨|上午"
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
        ...
        rule = u"下午|午后|pm|PM|晚上|夜间|夜里|今晚|明晚|晚|夜里"
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
