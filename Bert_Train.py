...
import csv
import os
from Business.Intent.Out_Call import tokenization, modeling, optimization
import tensorflow as tf
from Setting import QNSetting
import pickle
...
class MyProcessor(DataProcessor):
    ...
    def read_txt(self, filepath):
        df = pd.read_csv(filepath, delimiter="\t", names=['labels', 'text'], header=None)
        df = shuffle(df)  # shuffle数据
        lines = []
        for data in df.iloc[:].itertuples():
            content = data.labels + "\t" + data.text
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self.read_txt(data_dir), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self.read_txt(data_dir), "dev")
...
def train(data_dir, trans_model_dir, output_dir):
    base_dir = QNSetting.read_ini("common", "data_dir") + 'Intent_Data/OutCall/'
    pretrain_dir = base_dir + 'pretrain/bert_large/'
    ...
    mkdir_dir = lambda x: os.makedirs(x) if not os.path.exists(x) else True  # 目录是否存在,不存在则创建
    mkdir_dir(trans_model_dir)
